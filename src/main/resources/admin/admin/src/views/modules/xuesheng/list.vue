<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','体育成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="tiyuchengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'体育成绩':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','体育成绩') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="tiyuchengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'体育成绩':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','体育成绩') && contents.tableBtnIcon == 0" type="success" size="mini" @click="tiyuchengjiCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'体育成绩':'' }}</el-button>
    ...
    <tiyuchengji-cross-add-or-update v-if="tiyuchengjiCrossAddOrUpdateFlag" :parent="this" ref="tiyuchengjiCrossaddOrUpdate"></tiyuchengji-cross-add-or-update>
...
import tiyuchengjiCrossAddOrUpdate from "../tiyuchengji/add-or-update";
export default {
  data() {
    return {
      banjiOptions: [],
      ...
      tiyuchengjiCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    tiyuchengjiCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.tiyuchengjiCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.tiyuchengjiCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.tiyuchengjiCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
